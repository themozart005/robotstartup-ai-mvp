...
const GamePhasePanel: React.FC<GamePhasePanelProps> = ({
  ...
  const renderBootstrapPhase = () => {
    ...
    // Check if bootstrap funding was already collected
    const fundingAlreadyCollected = player.cash > 500000;
    
    // Handle bootstrap button click with debouncing
    const handleCollectBootstrap = () => {
      // Prevent multiple clicks
      if (isCollecting || fundingAlreadyCollected) {
        console.log('🚫 Preventing duplicate click or already collected');
        return;
      }
      ...
      // Reset collecting state after a delay (in case move fails)
      setTimeout(() => {
    ...
    return (
      <div className="space-y-4">
        <div className="bg-gradient-to-r from-green-600 to-emerald-600 p-6 rounded-lg">
          ...
          {!fundingAlreadyCollected ? (
            <button
              onClick={handleCollectBootstrap}
              disabled={isCollecting}
              className={`w-full py-3 px-6 rounded-lg font-bold transition-all flex items-center justify-center gap-2 ${
                isCollecting 
                  ? 'bg-gray-400 text-gray-200 cursor-not-allowed'
                  : 'bg-white text-green-600 hover:bg-green-50 shadow-lg hover:shadow-xl cursor-pointer'
              }`}
            >
              {isCollecting ? (
                <>
                  <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-green-600"></div>
                  <span>Collecting Bootstrap Funding...</span>
                </>
              ) : (
                <>
                  <DollarSign size={20} />
                  <span>Collect $50,000 Bootstrap Funding</span>
                </>
              )}
            </button>
          ) : (
            <div className="w-full py-3 px-6 rounded-lg bg-green-700/30 border border-green-500/50">
              <div className="flex items-center justify-center gap-2 text-green-100">
                <CheckCircle size={20} />
                <span className="font-semibold">Bootstrap Funding Collected!</span>
              </div>
              <p className="text-center text-green-200 text-sm mt-1">
          ...
          {process.env.NODE_ENV === 'development' && (
            <div className="mt-2 text-xs text-green-200 bg-green-800/30 p-2 rounded">
              <div>Debug: Round {currentRound}, Phase: {normalizedPhase}</div>
              <div>Turn: {isMyTurn ? 'Yes' : 'No'}, Can Move: {canMakeMove ? 'Yes' : 'No'}</div>
              <div>Cash: ${player.cash}, Collecting: {isCollecting ? 'Yes' : 'No'}</div>
            </div>
          )}
        </div>
